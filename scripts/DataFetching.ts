...
import { getBadDataFiles, getBadDataSites, getDirectory, getFileContents, getInstrumentSite, getLatestTankEntry, getTankList, visit } from './APIRequests';
import { ParsedData, parseNotes, processVisits } from './Parsers';
...
export async function fetchCalendarData(setOnline: { (value: SetStateAction<boolean>): void; (arg0: Promise<boolean>): void; }, setMarkedDates: { (value: SetStateAction<{}>): void; (arg0: (prevmarkedDates: any) => any): void; }, markedDates: {}, visitDict: Map<any, any>) {
  try {
...
export async function fetchTank(tank, setNetworkStatus, setLocationValue, setCO2Value, setCH4Value, setFillIDValue, setPSIValue, setLatestEntry) {
    setNetworkStatus(await isConnected());
    if (tank) {
...
export async function fetchNotes(site: string, data: string[], route: routeProp, setData: { (value: SetStateAction<string[]>): void; (arg0: any): void; }) {
    if (site && !data && route) {
      try {
