...
// Gets the most recent tank id for each tank type and gets all of the pressures for that tank id
const groupTankData = (entries: Entry[]) => {
  ...
  entries.forEach((entry) => {
    ["low_cal", "mid_cal", "high_cal", "lts"].forEach((tankType) => {
      ...
      if (tank) {
        ...
        if (
          !tankTimestamps[tankType] ||
          new Date(time) > new Date(tankTimestamps[tankType])
        ) {
  ...
  entries.forEach((entry) => {
    ["low_cal", "mid_cal", "high_cal", "lts", "n2_pressure"].forEach(
      (tankType) => {
        if (tankType === "n2_pressure") {
          ...
          if (tank) {
            ...
            if (
              pressure !== null &&
              !isNaN(pressure) &&
              new Date(entry.time_in) > oneYearAgo
            ) {
              if (!tankMap["N2"]) {
                tankMap["N2"] = [];
              }
          ...
          if (tank && tank.id === latestTankByType[tankType]) {
            ...
            if (pressure !== null && !isNaN(pressure)) {
              ...
              tankMap[latestTankByType[tankType]].unshift({
                time: entry.time_in || "Unknown",
                pressure,
              });
            }
...
export default function Diagnostics({ navigation }: NavigationType) {
  ...
  // Opens url to diagnostic page of air.utah.edu
  const openURL = async () => {
    const url = `https://air.utah.edu/s/diagnostics/?_inputs_&remove_failed_qc=false&color_by=%22QAQC_Flag%22&dates=%5B%22${lastWeek}%22%2C%22${today}%22%5D&column=%22%22&lvl=%22%22&instrument=%22%22&stid=%22${siteName}%22&submit=0&sidebarCollapsed=false&sidebarItemExpanded=null`;
  ...
  const [tankData, setTankData] = useState<
    Record<string, { time: string; pressure: number }[]>
  >({});

  // Get pressure data for each tank grouped by tank type
  useEffect(() => {
    if (data) {
      ...
      >
        ...
        {Object.entries(tankData).map(([tankId, data]) => {
          const validData = data.filter(
            (d) => !isNaN(new Date(d.time).getTime())
          );
          ...
          let predictedZeroDate = new Date(
            validData[validData.length - 1].time
          );
          predictedZeroDate.setDate(
            predictedZeroDate.getDate() + daysUntilZero
          );
          ...
          return (
            <View key={tankId} style={{ marginVertical: 10 }}>
              ...
              >
                {`Predicted empty date: ${formatDate(
                  predictedZeroDate.toISOString()
                )}`}
              </Text>
                ...
                <View style={{ flex: 1, marginLeft: 10, height: 225 }}>
                  ...
                  <XAxis
                    ...
                    formatLabel={(value, index) => {
                      ...
                      if (
                        index === totalLabels - 1 &&
                        normalizedX[index] - normalizedX[0] > predictedX * 0.3
                      ) {
                        return formatDate(validData[index].time);
                      }
